...
# generate dictionary of genes to phenotype terms based on
# http://wormbase.caltech.edu:8080/wormbase/manual/geneenrichment/davidahypergeometrictest
# retain phenotype terms that have 300 or more genes annotated
...
my $solr_url = 'http://wobr.caltech.edu:8082/solr/phenotype/';

my $json = JSON->new->allow_nonref;

my $url = $solr_url . "select?qt=standard&indent=on&wt=json&version=2.2&fl=id&start=0&rows=0&q=document_category:bioentity&facet=true&facet.field=regulates_closure&facet.limit=-1&facet.mincount=300&facet.sort=count&fq=source:%22WB%22&fq=-qualifier:%22not%22";
